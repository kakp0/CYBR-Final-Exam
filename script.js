...
// Removed start2025Btn and start2024Btn as they are no longer in HTML
...
const highScoreDisplay = {
    ...
    // Removed '2025' and '2024' high score displays
};
...
let score = 0;
let quizResults = [];
let lastQuizType = 'full'; // Hardcode to 'full' as it's the only type now
...
function startQuiz(quizType) {
    ...
    lastQuizType = quizType; // This will always be 'full' now
    score = 0;
    currentQuestionIndex = 0;
    quizResults = [];

    // All questions are now loaded from 'quizData'
    const dataCombined = typeof quizData !== 'undefined' ? quizData : [];

    // Load all questions only once
    if (allQuestionsRaw.length === 0) {
        allQuestionsRaw = dataCombined;
         console.log(`DEBUG: Initial question load. Raw questions: ${allQuestionsRaw.length}`);
    }

    // Select questions based on quiz type (always 'full' now)
    let selectedQuestionsRaw = [];
    if (quizType === 'full') {
        selectedQuestionsRaw = allQuestionsRaw.length > 0 ? allQuestionsRaw : dataCombined;
    } else {
        // This block should ideally not be reached with only one button
        console.error("Invalid quiz type specified:", quizType);
        alert(`Error: Invalid quiz type. Please try again.`);
    ...
    if (questionsToUse.length === 0) {
        ...
        alert(`Error: No questions found for the 'CYBR371 Final Exam' test.`);
 ...
 * @param {string} quizType The type of quiz ('full' now).
...
function displayHighScores() {
    ...
    const quizTypes = ['full']; // Only 'full' type now
...
// Removed event listeners for start2025Btn and start2024Btn
